...
async def startup_event():
    ...
    entries = list_models()
    if not entries:
        logger.warning("No models configured in models.json; preloading DEFAULT_MODEL_ID.")
        from src.services.model_registry import pin_model

        model_service = ModelService.instance()
        default_id = get_default_model_id()
        try:
            logger.info("Preloading model %s (type=unknown)...", default_id)
            model_service.preload_resources(default_id)
            pin_model(default_id)
            logger.info("Model %s (type=unknown) loaded successfully", default_id)
        except Exception as exc:
            logger.error("Failed to preload model %s: %s", default_id, exc, exc_info=True)
            raise
        return
