// Spectrum visualization functionality
class SpectrumVisualizer {
    ...
    drawHSQC() {
        ...
        if (this.data.length >= 3 && this.data.length % 3 === 0) {
            ...
            points.forEach(point => {
                ...
                const radius = Math.max(2, (point.intensity / intensityMax) * 8);
        ...
        ctx.textAlign = 'center';
        ctx.fillText('H-1 Shift (ppm)', width/2, height - 10);
    ...
    drawH1D() {
        ...
        if (this.data.length > 0) {
            ...
            this.data.forEach((shift, index) => {
                ...
                ctx.lineTo(x, margin + plotHeight - 30);
                ctx.stroke();
                ...
                ctx.textAlign = 'center';
                ctx.fillText(shift.toFixed(1), x, margin + plotHeight - 35);
            });
        }
    ...
    drawC1D() {
        ...
        ctx.font = '12px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('C-13 Shift (ppm)', width/2, height - 10);
    }
    ...
    drawMassSpec() {
        ...
        if (this.data.length >= 2 && this.data.length % 2 === 0) {
            ...
            points.forEach(point => {
                ...
                ctx.fillRect(x - 2, margin + plotHeight - barHeight, 4, barHeight);
                
                // m/z label
                ctx.fillStyle = '#374151';
                ctx.font = '10px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
                ctx.textAlign = 'center';
                ctx.fillText(point.mz.toFixed(1), x, margin + plotHeight + 15);
            });
        }
