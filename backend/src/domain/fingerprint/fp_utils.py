...
def get_bitinfos(
    ...
    temp_smi = Chem.MolToSmiles(mol, canonical=True, isomericSmiles=False)
...
def build_rankingset_csr(
    ...
    for _, cols in rows_cols:
        ...
        # Precompute normalized value (1/sqrt(nnz))
    ...
    csr = torch.sparse_csr_tensor(crow, cols, vals, size=(num_rows, num_cols))
    csr = csr.coalesce() if hasattr(csr, "coalesce") else csr
    return csr
